package BabyryAPI::Web;
use strict;
use warnings;
use utf8;
use parent qw/BabyryAPI Amon2::Web/;
use File::Spec;
use Log::Minimal;
use Carp;
use BabyryAPI::Web::Root;
...
# dispatcher
use BabyryAPI::Web::Dispatcher;
sub dispatch {
    return ( BabyryAPI::Web::Dispatcher->dispatch($_[0]) or __exception("response is not generated") );
}
...
# setup view
use BabyryAPI::Web::View;
{
    sub create_view {
        my $view = BabyryAPI::Web::View->make_instance(__PACKAGE__);
        no warnings 'redefine';
        *BabyryAPI::Web::create_view = sub { $view }; # Class cache.
        $view
    }
}
...
__PACKAGE__->add_trigger(
    BEFORE_DISPATCH => sub {
        ...
        my $session_not_required_paths = BabyryAPI::Common->config->{session_not_required_paths} || [];
        ...
        my $base_info = BabyryAPI::Web::Root->new->certify($session_id);
        ...
        if ($c->stash->{session_have_to_update}) {
            ...
            my $new_session_id = BabyryAPI::Web::Root->new->update_session($c->stash->{user_id});
