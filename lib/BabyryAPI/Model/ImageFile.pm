package BabyryAPI::Model::ImageFile;
...
sub factory {
    ...
    croak sprintf('Invalid file type : %s', $params->{path})
        if ! grep { $format eq $_ } @{ BabyryAPI::Common->config->{allowed_image_format} };

    my $class_name = sprintf 'BabyryAPI::Model::ImageFile::%s', camelize($format);
