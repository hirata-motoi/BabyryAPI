package BabyryAPI::Web::Dispatcher;
use strict;
use warnings;
use utf8;

use BabyryAPI::Web::Router;
...
my $router = BabyryAPI::Web::Router->new;
{
    for my $pl (dir(BabyryAPI->base_dir . "/route")->children) {
...
{ # for access log
    my $fh = File::Stamped->new(pattern => BabyryAPI->base_dir . "/log/access.log.%Y%m%d");
...
sub dispatch {
    ...
    my $web_root = $ENV{DEBUG_WEB_ROOT} || ref BabyryAPI->context;
    my $req      = $c->request;
...
sub _filter {
    ...
    for my $filter ( @$filters ) {
        ...
        my $res_type = ref $res eq 'Amon2::Web::Response'  ? FILTER_RESULT_RESPONSE  :
                       ref $res eq 'BabyryAPI::FormValidator' ? FILTER_RESULT_VALIDATOR : '';
        $results->{$res_type} = $res;
    }

    $results->{ FILTER_RESULT_VALIDATOR() } ||= BabyryAPI::FormValidator->new($c->req);
    return $results;
}
