...
use Test::BabyryAPI;
use Test::BabyryAPI::DBHResolver;
...
sub new {
    ...
    Test::BabyryAPI::DBHResolver->load($config || __config_file($service));

    bless {
        region   => $region,
        dbh      => Test::BabyryAPI::DBHResolver->resolver($name),
        database => Test::BabyryAPI::DBHResolver->database($name),
        tables   => $tables  || [],
        data     => $fixture || [],
        schema   => "",
        fixture  => {},
...
sub __config_file {
    ...
    sprintf("%s/config/db/dev.yaml", BabyryAPI->base_dir, $env);
}
...
sub dump {
    ...
    my $db = $self->{database};
    my $module_name = Test::BabyryAPI->module_name('schema', $self->{region}, $db);
    my $path = Test::BabyryAPI->module_path('schema', $self->{region}, $db);
    my $text = __module_text($module_name, $self->{schema});
    __create_module($path, $text);

    for my $tbl (keys %{ $self->{fixture} }) {
        my $module_name = Test::BabyryAPI->module_name('fixture', $self->{region}, $db, $tbl);
        my $path = Test::BabyryAPI->module_path('fixture', $self->{region}, $db, $tbl);
...
use strict;
use warnings;
use parent qw/Test::BabyryAPI::Reader/;
...
sub _create_teng_schema {
    ...
    my $module_name = Test::BabyryAPI->module_name(@ns);
    my $module_path = Test::BabyryAPI->module_path(@ns);
...
MODULE
    ...
    my $schema_path = Test::BabyryAPI->module_path(@ns, 'Schema');
