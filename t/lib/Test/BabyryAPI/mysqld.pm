...
our $MY_CNF    = $ENV{PROTOSERVER_MY_CNF} || Test::BabyryAPI->base_dir . '/my.cnf';
...
sub create_database {
    ...
    Test::BabyryAPI->diag("Created database: $dbname");
...
sub do_sql {
    ...
    Test::BabyryAPI->diag("Loading SQL to $dbname");

    my $dbh = $self->dbh($dbname);
    chomp $sql;
    map { $dbh->do($_) } grep { $_ =~ /.+/ } split(/;/, $sql);

    Test::BabyryAPI->diag("Loaded SQL to $dbname");
}

sub _create_q4m_functions {
    my ($self, $dbh, $dbname) = @_;

    Test::BabyryAPI->diag("Install Q4M plugin: $dbname");
...
sub cleanup {
    ...
    for my $db (@$rows) {
        ...
        Test::BabyryAPI->diag("Removed database: $db");
    }
}
